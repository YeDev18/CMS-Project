...
const useFilter = (data: any, label: string) => {
  ...
  if (portA) {
    FinalData = dataFinal.filter(
      (val: any) => val.tonnage_dt.port_dt_tonnage === `Port Autonome d'Abidjan`
    );
  } else if (portSP) {
    FinalData = dataFinal.filter(
      (val: any) =>
        val.tonnage_dt.port_dt_tonnage === `Port Autonome de San Pedro`
    );
  } else {
    <></>;
  }

  let dataFinalChecked = FinalData;
  if (correct) {
    dataFinalChecked = FinalData.filter(
